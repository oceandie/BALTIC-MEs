...
#=======================================================================================
def generate_loc_area_gs(da_bathy, max_dep):

    # BATHYMETRY
    msk_val = -99
    s_msk1   = da_bathy.where(da_bathy > max_dep, msk_val).data

    # POLYGON
    LON = [-81., -83.5, -74. , -65.5, -65.5]
    LAT = [ 26.,  35. ,  43.6,  38.0,  26. ]

    Is = []
    Js = []
    for n in range(len(LON)):
        j, i = get_ij_from_lon_lat(LON[n], LAT[n],
               da_bathy.nav_lon, da_bathy.nav_lat)
        Js.append(j)
        Is.append(i)

    s_msk2 = np.copy(da_bathy.data)
    area_j, area_i = get_poly_area_ij(Is, Js, s_msk2.shape)
    s_msk2[area_j, area_i] = msk_val

    # COMBINING
    s_msk = da_bathy * 0.
    msk = s_msk.data
    cond = np.logical_and(s_msk1==msk_val,s_msk2==msk_val)
    msk[cond] = 1.
    s_msk.data = msk
...
def generate_loc_area_gs_ext(da_bathy, max_dep, polygon):

    msk_val = -99

    if da_bathy.shape == (1207,1442):
       # orca025
       i_start = 830
       j_start = 822
    elif da_bathy.shape == (3606,4322):
       # orca12
       i_start = 2500
       j_start = 2465
    else:
    ...
    # POLYGON
    if polygon == 1: # no Labrador Sea
       LON = [-81., -83.5, -71.5 , -70.5, -48.0, -40.0,-53.0,-81.0]
       LAT = [ 26.,  35. ,  45.5,   50.0,  56.0,  47.0, 26.0, 26.0]
    elif polygon == 2: # yes Labrador Sea
       LON = [-81., -83.5, -71.5 , -71.0, -46.0, -39.0,-51.0,-81.0]
       LAT = [ 26.,  35. ,  45.5,   67.0,  66.0,  47.0, 26.0, 26.0]
    ...
    msk_poly = np.copy(da_bathy)*0.
    area_j, area_i = get_poly_area_ij(Is, Js, msk_poly.shape)
    msk_poly[area_j, area_i] = 1

    # BATHYMETRY
    bat_poly = xr.where(msk_poly == 1, da_bathy, msk_val)
    s_msk = xr.where(np.logical_and(bat_poly < max_dep, bat_poly > 0.), 1., 0.)

    # ONLY COASTAL PART
    msk = s_msk.data
    msk_fill = floodfill(msk, j_start, i_start, 0.0, -1.)
